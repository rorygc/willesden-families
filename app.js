...
const DATA = {
  thisWeek: [
    {
      ...
      cost: 'varies',
      area: 'Brent Cross',
      eventDate: 'Fri 17 Jul',
      eventTime: 'drop-in',
      blurb: 'Outdoor, child-led stay-and-play at Clitterhouse Farm. A gentle Friday activity for under-5s with space to explore nature and socialise.',
      recommendation: 'A simple, low-pressure option for little ones — and it runs every Friday, so it is easy to remember.',
    ...
    {
      id: 'event-ouryard-cafe-garden',
      section: 'this-week',
      name: 'Our Yard Café & Garden',
      age: 'all ages',
      cost: 'varies',
      area: 'Brent Cross',
      eventDate: 'Thu 16 Jul & Sat 18 Jul',
      eventTime: '10:00–16:00',
      blurb: 'Family-friendly café and garden stop at Clitterhouse Farm. Good for coffee, breakfast, lunch, and a relaxed outdoor break.',
      recommendation: 'Handy for a slow morning or an after-play refuel — and a nice backup when the weather is mixed.',
      tags: ['café', 'garden', 'food', 'indoor', 'outdoor'],
      contact: 'https://www.ouryard.org/our-cafe',
      linkLabel: 'Our Café',
      lat: 51.507,
      lng: -0.128,
      mapQuery: 'Our Yard at Clitterhouse Farm, Claremont Road, London NW2',
      promoImage: 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=800&h=450&fit=crop&auto=format',
      featured: true,
      region: 'local',
      cardTone: 'weekly'
    },
    {
      ...
      section: 'this-week',
      name: 'Roundwood Park Family Funfair',
      age: 'all ages',
      cost: 'varies',
      area: 'Harlesden',
      eventDate: 'Fri 10 – Sun 19 Jul',
      eventTime: 'afternoon–evening',
      blurb: 'Irvin Leisure funfair in Roundwood Park with rides, games, and sweet treats. A classic summer outing for Harlesden families.',
      recommendation: 'Best for an early evening visit or a quieter weekend slot before it gets busy.',
      tags: ['funfair', 'rides', 'outdoor', 'family', 'harlesden', 'summer'],
      contact: 'https://irvinleisure.co.uk/upcoming-funfairs/roundwood-park-family-funfair/',
      linkLabel: 'Funfair page',
      lat: 51.543,
      lng: -0.245,
  ...
  camps: [
    ...
    },
    {
      ...
      section: 'camps',
      name: 'Little House of Science — summer science camps',
      age: '4–12',
      cost: 'high',
      area: 'Hampstead (NW3)',
      blurb: 'Summer science camps running weekly 6 Jul – 28 Aug 2026 at venues in Hampstead. Eight themed weeks: physics, geography, biology, engineering, architecture, zoology, chemistry, and problem-solving. £119/day or £580/week with 10% sibling discount. Proper hands-on experiments — kids come home buzzing.',
      recommendation: 'Brilliant for curious kids — the experiments are genuinely exciting and staff are passionate scientists. Proper kit, real experiments.',
      tags: ['stem', 'science', 'experiments', 'hampstead', 'hands-on'],
      contact: 'https://portal.littlehouseofscience.com/classes?term=Summer%20camps',
    ...
    },
    {
      ...
      cost: 'medium',
      area: 'Brondesbury',
      blurb: 'Weekly tennis classes and multi-sport holiday camps in Brondesbury (NW2 2EB). Mix of tennis coaching, team sports, and games in a structured camp format. Book by the day or week.',
      recommendation: 'Solid local option for tennis-focused holiday cover — combines skill-building with multi-sport variety.',
  ...
  cafes: [
    ...
    {
      id: 'cafe-merals-bistro',
      section: 'cafes',
      name: "Meral's Café Bistro",
      age: 'all ages',
      cost: 'low',
      area: 'Cricklewood',
      blurb: 'A Cricklewood breakfast-and-coffee spot with a proper neighbourhood feel.',
      recommendation: 'Best in Cricklewood for breakfast and coffee.',
      tags: ['breakfast', 'coffee', 'café', 'Cricklewood'],
      contact: 'https://www.google.com/maps/search/?api=1&query=Meral%27s+Caf%C3%A9+Bistro+Cricklewood',
      linkLabel: 'Open in Maps',
      lat: 51.555,
      lng: -0.218,
      mapQuery: "Meral's Café Bistro, Cricklewood, London",
      featured: true,
      cardTone: 'weekly'
    },
    {
  ...
  indoor: [
    ...
    },
    {
      ...
      section: 'indoor',
      name: "Sammy's Soft Play",
      age: '0–5',
      cost: 'low',
      area: "Queen's Park",
      blurb: "Dedicated soft play for babies and toddlers in Queen's Park. Smaller and calmer than the big centres — perfect for crawlers and early walkers. Drop-in sessions, no need to book. Café on site with proper coffee.",
      recommendation: "The go-to for under-5s in Queen's Park — small enough that you can see your child from the café seating.",
      tags: ['soft play', 'toddler', 'baby', 'drop-in', 'queens park'],
      contact: 'https://www.sammyssoftplay.co.uk/',
      linkLabel: "Sammy's",
      lat: 51.535,
      lng: -0.205,
...
function getActiveMapCategories() {
  const toggles = document.getElementById('map-category-toggles');
  if (!toggles) return new Set(SECTIONS.map(s => dataKeyForSection(s)));
  const checked = toggles.querySelectorAll('input[data-map-category]:checked');
  return new Set([...checked].map(cb => cb.dataset.mapCategory));
}

function updateMapCategories() {
  // Re-run map with current category filters
  updateMap(currentFilteredGroups || getFilteredItems());
}
...
async function updateMap(filtered) {
  ...
  const combined = Object.values(filtered).flat().filter(item => activeCategories.has(item.section));
